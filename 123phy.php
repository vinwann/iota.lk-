...
$sql = "SELECT * From phy123posting_mcqs WHERE school='$school' AND paper_yr='$paper_yr'";
$result = mysqli_query($conn,$sql);
$queryResults = mysqli_num_rows($result);
if ($queryResults > 0 ) {
while ($row = mysqli_fetch_assoc($result)) {
echo "<title>".$row['school']." ".$row['paper_yr']." grade 12 third term test physics MCQ answers </title>
   <meta name='description' content='Accurate MCQ answers for ".$row['school']." ".$row['paper_yr']." grade 12 third term test physics paper'>";
}
}
                }?>
</head>
