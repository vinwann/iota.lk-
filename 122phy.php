...
$sql = "SELECT * From phy122posting_mcqs WHERE school='$school' AND paper_yr='$paper_yr'";
$result = mysqli_query($conn,$sql);
$queryResults = mysqli_num_rows($result);
if ($queryResults > 0 ) {
while ($row = mysqli_fetch_assoc($result)) {
echo "<title>".$row['school']." ".$row['paper_yr']." grade 12 second term test physics MCQ answers </title>
   <meta name='description' content='Accurate MCQ answers for ".$row['school']." ".$row['paper_yr']." grade 12 second term test physics paper'>";
}
}
                }?>
</head>
...
(adsbygoogle = window.adsbygoogle || []).push({});
                  ...
                  <form method='POST' action='includes/12secondphyrec_ans.php'>
