...
?>
<title>Download A/Ls-grade 13 third term test chemistry papers of Royal college,Visakha Vidayalya,Ananda college,Devi Balika Vidyalaya etc</title>
    <meta name="description" content="Download A/Ls-grade 13 third term test MCQ and ESSAY chemistry papers of Royal college,Visakha Vidayalya,Ananda college,Devi Balika Vidyalaya etc ">
...
<ul id="newpapers">
			<li>Ananda college 2014 ESSAY paper</li>
			<li>Royal college 2011 MCQ paper </li>
			<li></li>
			<li></li>
		</ul></div>
...
<style>
	...
	<div id="searchcon">
		...
		<form action="chemtt133search" method="POST">
...
(adsbygoogle = window.adsbygoogle || []).push({});
 ...
 <?php 
 $sql = "SELECT * From chem133 ORDER BY paper_yr";
	$result = mysqli_query($conn,$sql);
$queryResults = mysqli_num_rows($result);
	echo "<p id='all'> All results (".$queryResults.")</p>";
 ?>
	<?php
	$sql = "SELECT * From chem133 ORDER BY paper_yr DESC";
	$result = mysqli_query($conn,$sql);
	$queryResults = mysqli_num_rows($result);
	if ($queryResults > 0 ) {
		while ($row = mysqli_fetch_assoc($result)) {
		   ...
		   <div class='mcqBox'>
                <h1>".$row['school'] . "  ".$row['paper_yr']." <br>
                 ...
                 <a href='chem13third/chem133".$row['school']."".$row['paper_yr']."".$row['papert']."IOTA.pdf' class='mcqAcn' download><div class='contentbox'>Download</div></a>
                 <a href='133chemtt?school=".$row['school']."&paper_yr=".$row['paper_yr']."&papert=".$row['papert']."' class='mcqAcn' ><div class='contentbox'>View</div></a>
	        </div>";
		}
