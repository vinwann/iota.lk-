...
?>
<title>Download A/Ls-grade 13 first term test physics papers of Royal college,Visakha Vidayalya,Ananda college,Devi Balika Vidyalaya etc</title>
    <meta name="description" content="Download A/Ls-grade 13 first term test MCQ and ESSAY physics papers of Royal college,Visakha Vidayalya,Ananda college,Devi Balika Vidyalaya etc ">
...
?>
	<div id="searchcon">
		...
		<form action="phytt131search" method="POST">
...
(adsbygoogle = window.adsbygoogle || []).push({});
   ...
   <?php 
 $sql = "SELECT * From phy131 ORDER BY paper_yr";
	$result = mysqli_query($conn,$sql);
$queryResults = mysqli_num_rows($result);
	echo "<p id='all'> All results (".$queryResults.")</p>";
 ?>
	<?php
	$sql = "SELECT * From phy131 ORDER BY paper_yr DESC";
	$result = mysqli_query($conn,$sql);
	$queryResults = mysqli_num_rows($result);
	if ($queryResults > 0 ) {
		...
		<div class='mcqBox'>
                <h1>".$row['school'] . "  ".$row['paper_yr']." <br>
                 ...
                 <a href='phy13first/phy131".$row['school']."".$row['paper_yr']."".$row['papert']."IOTA.pdf' class='mcqAcn' download><div class='contentbox'>Download</div></a>
                 <a href='131phytt?school=".$row['school']."&paper_yr=".$row['paper_yr']."&papert=".$row['papert']."' class='mcqAcn' ><div class='contentbox'>View</div></a>
	        </div>";
		}
